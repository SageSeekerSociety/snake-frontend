...
// 更新蛇列表（用于占位符解析）
const updateSnakesList = (snakes: Snake[]) => {
  if (Array.isArray(snakes)) {
    allSnakes.value = [...snakes];
  }
};
...
const setupEventListeners = () => {
  ...
  eventBus.on(GameEventType.UI_UPDATE_SCOREBOARD, updateSnakesList);
  ...
  return () => {
    ...
    eventBus.off(GameEventType.UI_UPDATE_SCOREBOARD, updateSnakesList);
  };
};
...
.notification-item.replay-game-item {
  ...
  /* 浅蓝色边框 */
  background-color: rgba(160, 216, 239, 0.07);
  /* 浅蓝色背景 */
}
