...
export class SnakeRenderer implements EntityRenderer<Snake> {
  ...
  private drawSnakeHead(
    ...
    if (direction === Direction.RIGHT) {
      ctx.fillRect(
        head.x + size - eyeOffset + eyeAdjustX,
        head.y + eyeOffset + eyeAdjustY,
        eyeSize,
        eyeSize
      );
      ctx.fillRect(
        head.x + size - eyeOffset + eyeAdjustX,
        head.y + size - eyeOffset - eyeSize + eyeAdjustY,
        eyeSize,
        eyeSize
      );
    } else if (direction === Direction.LEFT) {
      ctx.fillRect(
        head.x + eyeOffset - eyeSize + eyeAdjustX,
        head.y + eyeOffset + eyeAdjustY,
        eyeSize,
        eyeSize
      );
      ...
      ctx.fillRect(
        head.x + eyeOffset + eyeAdjustX,
        head.y + eyeOffset - eyeSize + eyeAdjustY,
        eyeSize,
        eyeSize
      );
      ctx.fillRect(
        head.x + size - eyeOffset - eyeSize + eyeAdjustX,
        head.y + eyeOffset - eyeSize + eyeAdjustY,
        eyeSize,
        eyeSize
      );
      ...
      ctx.fillRect(
        head.x + eyeOffset + eyeAdjustX,
        head.y + size - eyeOffset + eyeAdjustY,
        eyeSize,
        eyeSize
      );
      ctx.fillRect(
  ...
  private drawHeadMatchNumber(
    ...
    try {
      ...
      const raw =
        typeof meta.matchNumber === "number" && meta.matchNumber > 0
          ? meta.matchNumber
          : null;
      if (!raw) return;
  ...
  private drawSnakeTail(
    ...
    if (isMoving) {
      ...
      // 绘制拉伸的尾部
      ctx.fillRect(tail.x + offsetX, tail.y + offsetY, tailWidth, tailHeight);
    } else {
      // 正常状态下的尾部绘制
      ctx.fillRect(
        tail.x + tailPadding,
  ...
  private renderDeathAnimation(
    ...
    for (let i = 0; i < body.length; i++) {
      ...
      // 计算扭曲和抖动效果
      const oscillation =
        Math.sin(deathProgress * Math.PI * 10 + i * 0.5) * deathProgress * 5;
      ...
      ctx.fillRect(segment.x + offsetX, segment.y + offsetY, newSize, newSize);
